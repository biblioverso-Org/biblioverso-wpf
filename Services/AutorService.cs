...
namespace library.Services
{
    public class AutorService
    {
        ...
        public async Task<IEnumerable<Autor>> GetAllAsync()
        {
            ...
            var query = @"
        SELECT id_autor, nombre, nacionalidad, fecha_nac, fecha_muerte, biografia, foto
        FROM autor
        ORDER BY id_autor";
            ...
            while (await reader.ReadAsync())
            {
                autores.Add(new Autor
                {
                    ...
                    Foto = reader.IsDBNull(6) ? null : reader.GetString(6) // ✅ Nuevo campo
                });
            }
        ...
        {
            var query = @"
        INSERT INTO autor (nombre, nacionalidad, fecha_nac, fecha_muerte, biografia, foto)
        VALUES (@nombre, @nacionalidad, @fecha_nac, @fecha_muerte, @biografia, @foto)";
        ...
        new NpgsqlParameter("foto", (object?)autor.Foto ?? DBNull.Value)
    };
        ...
        new NpgsqlParameter("fecha_muerte", (object?)autor.FechaMuerte ?? DBNull.Value),
        new NpgsqlParameter("biografia", (object?)autor.Biografia ?? DBNull.Value),
        new NpgsqlParameter("foto", (object?)autor.Foto ?? DBNull.Value) // ✅ Nuevo parámetro
    };
